...
"""
        ...
        # 连续性（需求③）：把上一时段个股结论注入到本时段个股 prompt，
        # 让分析建立在已有结论之上，减少独立重算造成的重复；缺标的原则标「暂无数据」。
        previous_slot_stock_section = format_previous_slot_stock_conclusions_prompt_section(
            context.get("previous_slot_stock_conclusions"),
            report_language=report_language,
        )
