...
from src.services.stock_continuity import (
    ...
    load_previous_slot_stock_conclusions,
)
...
class StockAnalysisPipeline:
    ...
    def _without_runtime_prompt_context(context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(enhanced_context, dict):
            ...
            # 连续性（需求③）：上一时段个股结论仅作为运行时 prompt 输入，不入历史快照。
            enhanced_context.pop("previous_slot_stock_conclusions", None)
    ...
    def process_single_stock(
        ...
        try:
            ...
            if previous_slot_stock_conclusions is not None:
                analyze_kwargs["previous_slot_stock_conclusions"] = previous_slot_stock_conclusions
    ...
    def run(
        ...
        # 连续性（需求③）：在本轮开始时一次性读取每只持仓的上一时段个股结论，
        # 复用既有 analysis_history 存储。该字典以只读方式共享给各并发 worker，
        # 不会在分析中写入，因此无需加锁。缺某标的时映射为 None（prompt 中标「暂无数据」）。
        previous_slot_stock_conclusions = load_previous_slot_stock_conclusions(
            self.db, stock_codes, days=2
        )
