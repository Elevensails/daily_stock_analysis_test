...
# system prompt：定义改写员角色与硬约束（grounding + 红线 + 自洽）。
REPAIR_SYSTEM: str = """你是一名严谨的 A股分析报告**改写员**。你的任务是根据「校验反馈」修正一份被防幻觉 gate 拦截的报告，**整篇重新生成**一份干净、合规的报告。
...
4. 仅按反馈修正违规处，**保持其余内容、结构与风格不变**，不要自由发挥、不要新增未举证断言。
5. 直接输出修正后的完整 markdown 报告，不要任何解释、前缀或元评论。"""

# user prompt 模板：用字符串替换注入 {report_kind} / {reasons} / {source_facts_block} / {original_text}。
# 不使用 str.format，避免正文/反馈中的花括号触发格式化错误。
REPAIR_USER_TEMPLATE: str = """# 任务
下面是一份被校验 gate 拦截的{report_kind}分析报告。请基于「校验反馈」与（可选的）「行情源 fact sheet」，对违规处进行修正，**整篇重新生成**一份干净报告。
...
# 原始报告（供参考，修正时保持非违规内容不变）
{original_text}

# 输出要求
直接输出修正后的完整 markdown 报告，不要任何解释或前缀。"""
