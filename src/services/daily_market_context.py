...
def format_daily_market_context_prompt_section(
    ...
    *,
    ...
    if language in ("en", "ko"):
        ...
        lines.append("- Anti-hallucination: only answer based on facts in the historical context above; never fabricate any price, change %, number, or symbol. If the context lacks a data point for a symbol, mark it '暂无数据 (no data)' and do not infer.")
        lines.append("- Output: structured output (per-symbol sections). Self-check before finalizing: every number you emit must be traceable to the context above.")
        if source:
    ...
    lines.append("- 防幻觉约束：仅依据上方【历史上下文】中的事实作答，不得编造任何价格、涨跌幅、数值或标的；若上下文未提供某标的的某项数据，明确标注「暂无数据」，禁止推断。")
    lines.append("- 输出要求：结构化输出（按标的分节，便于核对），输出前自校验——每条数字都必须在上下文中有据可查。")
    if source:
